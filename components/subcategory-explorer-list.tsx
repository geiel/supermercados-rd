...
type SubCategoryExplorerListProps = {
  categoryId: number;
  subCategoryId: number;
  groups: Array<{ id: number; name: string; humanId: string }>;
  initialProducts: GroupExplorerProduct[];
  total: number;
  initialOffset: number;
};

export function SubCategoryExplorerList({
  categoryId,
  subCategoryId,
  groups,
  initialProducts,
  ...
  const lastSubCategoryRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    if (lastSubCategoryRef.current === subCategoryId) {
      return;
    }

    lastSubCategoryRef.current = subCategoryId;
    ...
    setIsSortOpen(false);
  }, [initialOffset, initialProducts, subCategoryId, total]);
  ...
  const loadPage = useCallback(
    async (
      ...
      const response = await fetch(
        `/api/explorar/${categoryId}/${subCategoryId}/products?${params.toString()}`,
        { cache: "no-store" }
      );
      ...
      return (await response.json()) as SubCategoryExplorerResponse;
    },
  ...
  const loadFirstPage = useCallback(
    async (targetSort: GroupExplorerSort) => {
      ...
      try {
        ...
        console.error("[subcategory-explorer] Failed to load products", error);
  ...
  const handleLoadMore = useCallback(async () => {
    ...
    try {
      ...
      console.error("[subcategory-explorer] Failed to load more", error);
      setErrorMessage("No se pudieron cargar más productos.");
    } finally {
      if (requestId === requestIdRef.current) {
        isLoadingRef.current = false;
    ...
    <>
      {groups.length > 0 ? (
        <div className="px-2 md:px-0">
          <GroupsFilter groups={groups} asLinks />
        </div>
      ) : null}

      <div className="flex items-center justify-end pb-2 px-2 md:px-0">
        {isMobile ? (
      ...
      {products.length === 0 && isLoading ? (
        ...
        <div className="grid grid-cols-2 place-items-stretch md:grid-cols-3 lg:grid-cols-5">
          {products.map((product) => (
            <SubCategoryExplorerCard key={product.id} product={product} />
          ))}
        </div>
      )}
...
function SubCategoryExplorerCard({ product }: { product: GroupExplorerProduct }) {
  return (
      ...
      >
        <div className="flex justify-center">
          <div className="h-[220px] w-[220px] relative">
            <SubCategoryExplorerImage product={product} />
          </div>
        </div>
...
function SubCategoryExplorerImage({ product }: { product: GroupExplorerProduct }) {
  if (!product.image) {
    return (
      <Image
