...
import { normalizeTerm } from "@/components/autocomplete";
...
import { DEALS_DISCOUNT_OPTIONS } from "@/types/deals";

type BrandExplorerFiltersProps = {
  brandId: number;
...
export function BrandExplorerFilters({
  brandId,
  variant = "auto",
}: BrandExplorerFiltersProps) {
  if (variant === "mobile") {
    return <MobileFilters brandId={brandId} />;
  }

  if (variant === "desktop") {
    return <DesktopFilters brandId={brandId} />;
  }

  return (
    <>
      <div className="lg:hidden">
        <MobileFilters brandId={brandId} />
      </div>
      <DesktopFilters brandId={brandId} />
    </>
  );
}

export function BrandExplorerActiveFilters({
  brandId,
  className,
}: {
  brandId: number;
  ...
  const currentShopIds = useMemo(
    () => parseCommaSeparatedInts(searchParams.get("shop_ids")),
    [searchParams]
  );
  ...
  const shouldFetchCategories = currentGroupIds.length > 0;

  const { data: shops } = useQuery<DealsShopOption[]>({
    queryKey: ["brand-shops", brandId, "active"],
    queryFn: async () => {
      const res = await fetch(`/api/brands/${brandId}/shops`);
  ...
  const { data: categories } = useQuery<DealsGroupOption[]>({
    queryKey: ["brand-categories", brandId, "active"],
    queryFn: async () => {
    ...
    <div className={cn("flex flex-wrap gap-2", className)}>
      {currentGroupIds.map((groupId) => (
        <button
          key={`group-${groupId}`}
          type="button"
          onClick={() => handleRemoveCategory(groupId)}
          ...
          aria-label={`Quitar categoria ${categoriesById.get(groupId) ?? groupId} del filtro`}
        >
          <span className="max-w-[200px] truncate text-left">
            {categoriesById.get(groupId) ?? `Categoria ${groupId}`}
          </span>
      ...
      {discountLabel ? (
        ...
        >
          <span className="max-w-[240px] truncate text-left">{discountLabel}</span>
          <X className="h-3.5 w-3.5 shrink-0 opacity-70 transition-opacity group-hover:opacity-100" />
        </button>
      ) : null}
    </div>
...
function MobileFilters({ brandId }: { brandId: number }) {
  ...
  return (
    <Drawer open={isOpen} onOpenChange={setIsOpen} repositionInputs={false}>
      ...
      <DrawerContent className="max-h-[85vh]">
        ...
        <BrandExplorerActiveFilters brandId={brandId} className="px-4 pt-3" />
        <div className="p-4 max-h-[calc(100vh-10rem)] overflow-y-auto">
          <FilterSections brandId={brandId} />
        </div>
...
function DesktopFilters({ brandId }: { brandId: number }) {
  return (
...
function FilterSections({ brandId }: { brandId: number }) {
  return (
    <div className="space-y-6 py-4 md:pr-4">
      <CategoryFilterSection brandId={brandId} />
      <BrandPriceFilterSection brandId={brandId} />
      <DiscountFilterSection brandId={brandId} />
      <ShopsFilterSection brandId={brandId} />
    </div>
  );
}
...
function CategoryFilterSection({ brandId }: { brandId: number }) {
  ...
  const { data: categories, isLoading } = useQuery<DealsGroupOption[]>({
    queryKey: ["brand-categories", brandId, categoriesParams],
    queryFn: async () => {
      const url = categoriesParams
        ? `/api/brands/${brandId}/categories?${categoriesParams}`
  ...
  const filteredCategories = useMemo(() => {
    ...
    return categories.filter((category) => {
      const cleanName = normalizeTerm(category.name);
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    ...
                    toggleCategory(category.id, !isSelected);
                  }}
...
function BrandPriceFilterSection({ brandId }: { brandId: number }) {
  const [isExpanded, setIsExpanded] = useState(true);
  const searchParams = useSearchParams();
  const router = useRouter();
  const pathname = usePathname();
  ...
  const { data: priceStats, isLoading } = useQuery<DealsPriceStatsResponse>({
    queryKey: ["brand-price-stats", brandId, priceStatsParams],
    queryFn: async () => {
      const url = priceStatsParams
        ? `/api/brands/${brandId}/price-stats?${priceStatsParams}`
        : `/api/brands/${brandId}/price-stats`;
      const res = await fetch(url);
...
function DiscountFilterSection({ brandId }: { brandId: number }) {
  const [isExpanded, setIsExpanded] = useState(true);
  const searchParams = useSearchParams();
  const router = useRouter();
  const pathname = usePathname();
  ...
  const { data: discounts, isLoading } = useQuery<DealsDiscountOption[]>({
    queryKey: ["brand-discounts", brandId, discountsParams],
    queryFn: async () => {
      const url = discountsParams
        ? `/api/brands/${brandId}/discounts?${discountsParams}`
        : `/api/brands/${brandId}/discounts`;
      const res = await fetch(url);
...
function ShopsFilterSection({ brandId }: { brandId: number }) {
  const [isExpanded, setIsExpanded] = useState(true);
  const [showAll, setShowAll] = useState(false);
  const searchParams = useSearchParams();
  const router = useRouter();
  ...
  const currentShopIds = useMemo(
    () => new Set(parseCommaSeparatedInts(searchParams.get("shop_ids"))),
    [searchParams]
  );
  ...
  const { data: shops, isLoading } = useQuery<DealsShopOption[]>({
    queryKey: ["brand-shops", brandId, shopsParams],
    queryFn: async () => {
      const url = shopsParams
        ? `/api/brands/${brandId}/shops?${shopsParams}`
        : `/api/brands/${brandId}/shops`;
      const res = await fetch(url);
      if (!res.ok) throw new Error("Failed to fetch shops");
      return res.json();
    },
          ...
          {displayedShops?.map((shop) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-sm text-muted-foreground">{shop.count}</span>
              </div>
            );
          })}
          {hasMore && (
