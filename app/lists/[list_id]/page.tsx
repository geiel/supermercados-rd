...
export default async function Page({ params, searchParams }: Props) {
    ...
    if (!user) {
        return <div>Inicia sesión para ver tu lista.</div>;
    }
    ...
    const list = await db.query.list.findFirst({
        where: (list, { and, eq }) => and(eq(list.userId, user.id), eq(list.id, listId)),
        with: {
            items: true
        }
    });
    ...
    if (list.items.length === 0 && listGroupItems.length === 0) {
        return <div>Tu lista está vacía.</div>;
    }
    ...
    const getBestValueGroupPicks = (
        ...
        const unitLabel = (() => {
            ...
            if (hasWeight && hasVolume && hasLength) {
                return "100 GR/ML/M";
            }
